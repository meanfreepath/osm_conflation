...
public class Junction {
    ...
    protected List<PathSegment> determineOutgoingPathSegments(final RouteConflator routeConflator, final PathSegment incomingPathSegment) {
        ...
        for(final OSMWay containingWay : junctionNode.containingWays.values()) {
            ...
            if(incomingPathSegment == null || line.way.osm_id != incomingPathSegment.getLine().way.osm_id) { //i.e. first junction on a PathTree, or transitioning to a new line
                ...
                if (junctionNode == containingWay.getFirstNode()) { //junction is first node on way: just travel forward
                    checkCreateNewPathSegment(line, this, PathSegment.TravelDirection.forward, divergingPathSegments);
                } else if (junctionNode == containingWay.getLastNode()) { //junction is last node on way: just travel backward
                    checkCreateNewPathSegment(line, this, PathSegment.TravelDirection.backward, divergingPathSegments);
                ...
                if (junctionNode != containingWay.getFirstNode() && junctionNode != containingWay.getLastNode()) {
                    checkCreateNewPathSegment(line, this, incomingPathSegment.travelDirection, divergingPathSegments);
                }
            }
        }
        return divergingPathSegments;
    }
    private static void checkCreateNewPathSegment(final OSMWaySegments line, final Junction originJunction, final PathSegment.TravelDirection travelDirection, final List<PathSegment> divergingPathSegments) {
        ...
        divergingPathSegments.add(PathSegment.createNewPathSegment(line, originJunction, travelDirection));
    }
}
