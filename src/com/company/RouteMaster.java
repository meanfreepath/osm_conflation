...
public class RouteMaster {
    ...
    public void conflateStops(final double conflictDistance) throws InvalidArgumentException {
        if(allRouteStops.size() == 0) {
            return;
        }

        //fetch all possible useful ways that intersect the route's combined bounding box
        final double latitudeDelta = -0.5 * conflictDistance / Point.DEGREE_DISTANCE_AT_EQUATOR, longitudeDelta = latitudeDelta / Math.cos(Math.PI * roughCentroid.latitude / 180.0);
        final Region stopDownloadRegion = importRouteMaster.getBoundingBox().regionInset(latitudeDelta, longitudeDelta);
        final OverpassConverter converter = new OverpassConverter();
        try {
            final String query;
            switch (routeType) {
        ...
        for(final StopArea stop : allRouteStops.values()) {
